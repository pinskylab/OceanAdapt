...
# test1 <- anti_join(all_regions_spp, tax, by = c("spp"="taxon")) %>% 
#   # format the column into genus and spp so that I can check it against fishbase
#   separate(spp, into= c("genus", "spp", "extra", "extra1"), sep = " ")%>%
#   mutate(genus = str_to_title(genus),        ,
...
# not_in_tax$name <- paste(not_in_tax$genus, not_in_tax$spp)
# not_in_tax$name <- gsub(' NA', '', not_in_tax$name)
...
```{r}
#remove flagged spp
temp = list.files(here("flagspp"), pattern="*.csv")
setwd("~/Documents/GitHub/OceanAdapt/flagspp")
...
tax_added$name %in% flagspp$test.spp

#remove flagged spp from tax_added
test <- tax_added[!tax_added$name %in% flagspp$test.spp,]
print((nrow(tax_added) - nrow(test))/nrow(tax_added))

#4.9% of rows are removed
tax_added <- tax_added[!tax_added$name %in% flagspp$test.spp,]

#remove flagged spp from tax_added
test <- not_in_tax[!not_in_tax$spp %in% flagspp$test.spp,]
print((nrow(not_in_tax) - nrow(test))/nrow(not_in_tax))

#1.2% of unknown species dropped
not_in_tax <- not_in_tax[!not_in_tax$spp %in% flagspp$test.spp,]
print((nrow(not_in_tax) - nrow(test))/nrow(not_in_tax))
