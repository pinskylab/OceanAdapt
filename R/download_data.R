...
# FGP doesn't work on this version of R so I'm loading the function here:
...
get_DFO_REST <-function(service='ADAPT_Canada_Atlantic_Summer_2016', save_csv=TRUE, n_rec = 0) {
  ...
  if (save_csv){
    filename = paste0("data_raw/",service,".csv")
...
haul <- list.files(path = "~/Downloads", pattern = "EVENT", full = T)
 ...
 # Get Scotian data ====
  ...
  scot_sumr <- get_DFO_REST(service = "ADAPT_Canada_Atlantic_Summer_2016", save_csv = TRUE)
  scot_fall <- get_DFO_REST(service = "ADAPT_Canada_Atlantic_Fall_2016", save_csv = TRUE)
  scot_spr <- get_DFO_REST(service = "ADAPT_Canada_Atlantic_Spring_2016", save_csv = TRUE)
