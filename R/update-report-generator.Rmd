---
title: "OceanAdapt Update Report"
date: Sys.Date()
output: 
...
---
<!-- 
The purpose of this document is to compile the raw data into OceanAdapt website ready data and to knit an R script that can be used by NJAES to generate the website and maps.
-->

2019-03-21  I added 2018 data to the group and had to add 12 lines of new spp to the taxonomy file. Zipped the 3 largest files and added code to read from zipped csv.
...
# run all of the chunks but don't inlcude any output
knitr::opts_chunk$set(include = FALSE)
```

1. Run update.data.R  
```{r updata-data-script}
# = Load Packages ====
...
files <- list.files(full = TRUE, path = "~/Documents/OceanAdapt/data_updates/Data_Updated")
zip(basename(new.zip.folder), files=files)
...
```
- Errors:  
    - getwd/setwd at the beginning produced errors because the working directory was already "R".  
    - line 315 newGMEX produced the message EOF within quoted string.  
    - line 500 *zip(basename(new.zip.folder), files=list.files(basename(new.zip.folder),full=TRUE)) produced Error in zip(basename(new.zip.folder), files = list.files(basename(new.zip.folder), : 'files' must a character vector specifying one or more filepaths*, changed to literal path.
    - the way that the code is written caused the zip file to be written in the "R" folder.  Moved it by hand.
    
1. Run the complete_r_script.R
```{r complete_r_script}
